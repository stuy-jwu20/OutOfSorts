  ...
  public static void main(String[]args){
    if(args.length < 2){
      ...
      if(args.length < 3 || args[2].equals("random")){
          ...
          int currentSeedInt = (rng.nextInt() % 100);
          randData[i] = currentSeedInt;
        }
      }else if(args[2].equals("equal")){
        int constantValue = (int)(Math.random() * 100);
        for (int i = 0; i < Integer.parseInt(args[0]); i++) {
          randData[i] = constantValue;
        }
      }else if(args[2].equals("sorted")){
        int increasingValue = (int)(Math.random() * 10);
        for (int i = 0; i < Integer.parseInt(args[0]); i++) {
          randData[i] = increasingValue;
          increasingValue += (int)((Math.random() * 100));
        }
      }else if(args[2].equals("reversed")){
        int decreasingValue = (int)(Math.random() * 1000);
        for (int i = 0; i < Integer.parseInt(args[0]); i++) {
          randData[i] = decreasingValue;
          decreasingValue -= (int)((Math.random() * 100));
        }
      }
